// TRAIN_FUNCTIONS.H Rev: 02/28/23.
// Not a class, just a group of functions -- but must be #included in every Trains program.
// Declares and defines several functions that are global to all (or nearly all) Arduino modules.
// Any non-const global variables should be declared here as extern, and defined in the .ino or .cpp file.
...
extern Display_2004* pLCD2004;  // We must still declare/define pLCD2004 to point at the class in the main module i.e. O_LEG.
extern char lcdString[];  // Global array to hold strings sent to Digole 2004 LCD; last char is for null terminator.

// Centipede shift register is used herein by haltIfHaltPinPulledLow() and endWithFlashingLED() when called by (only) SWT and LEG.
// We define the Centipede pointer as extern/global here so we don't have to pass the pointer when calling haltIfHaltPinPulledLow()
// and endWithFlashingLED() from those or any other modules.
// Note: <Wire.h> is already included in <Centipede.h> so not needed here.
#include <Centipede.h>
extern Centipede* pShiftRegister;  // Needed to release SWT Turnout and LEG Accessory relays upon Emergency Stop
