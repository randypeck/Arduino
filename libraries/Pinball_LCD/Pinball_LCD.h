...
// Pinball_LCD is a child class of parent DigoleSerialDisp, in order to be able to make both of them static, via a
// single instantiation from the main program.
// 01/19/26: Added clearRow() function to clear a single row (1..4) by filling with spaces.
// 01/14/26: Added clearScreen() function to clear both buffers and physical display.
...
#ifndef PINBALL_LCD_H
#define PINBALL_LCD_H
...
class Pinball_LCD : private DigoleSerialDisp {

public:

  Pinball_LCD(HardwareSerial* t_hdwrSerial, long unsigned int t_baud);  // Constructor
  ...
  void clearScreen();
  void clearRow(const byte row);  // Clear a single row (1..4) by filling with spaces.
